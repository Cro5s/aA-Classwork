...
function reverseString(str) {
  ...
  const partial = strArray.slice(1).join("");

  return reverseString(partial).concat(strArray[0]);
}
...
function pathFinder(directories, targetFile) {
  ...
  for (let i = 0; i < keys.length; i++) {
    if (keys[i] === targetFile) {
      ...
      let check = pathFinder(directories[keys[i]], targetFile);

      if (check) {
        path = keys[i].concat(check);
        break;
      }
    }
  }
