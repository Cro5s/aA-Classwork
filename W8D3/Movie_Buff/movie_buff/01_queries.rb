...
def harrison_ford
  ...
  #  joins castings on movie.id = castings.movie_id AND castings.ord != 1
  <<-SQL
    SELECT movies.id, movies.title
    FROM movies
    JOIN castings ON movies.id = castings.movie_id AND castings.ord != 1
    JOIN actors ON castings.actor_id = actors.id
    WHERE actors.name = 'Harrison Ford';
  SQL

  # Actor.joins(:movies).where(actors: {name: 'Harrison Ford'}).where('castings.ord != 1' )
...
def biggest_cast
  ...
  #   limit 3
  # SQL
  # Actor.select('movies.id, movies.title').joins(:movies).group('movies.id').order('COUNT(*) DESC').limit(3)
end
...
def directed_by_one_of(them)
  ...
  <<-SQL
    ...
    from 
      movies
    join
      actors on movies.director_id = actors.id
    where
      actors.name in (
  ...
  Actor.select('movies.id, movies.title').joins(:directed_movies).where(name: them)
end
